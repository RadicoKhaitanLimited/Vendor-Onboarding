...
export default function EditOnboardingModal({
  ...
  const isPanInoperative = isPanNameEditable(data.pan_number) &&
    hasAny(panVerification?.verification_status, ['inoperative', 'not operative'])
  ...
  const validate = () => {
    ...
    if (!isCustomer) {
      if (!form.reference_vendor_code.trim()) e.reference_vendor_code = 'Business Partner Number is required.'
      if (!form.purchase_orgs_to_open.trim()) e.purchase_orgs_to_open = 'Purchase Org. is required.'
      if (!form.search_term) e.search_term = 'Search Term is required.'
      if (!form.company_code_to_open) e.company_code_to_open = 'Company Code is required.'
      if (!form.payment_terms) e.payment_terms = 'Payment Terms is required.'
      if (!form.tds_codes && !e.tds_codes) e.tds_codes = 'TDS Codes is required.'
    } else {
      if (!form.sales_reference_orgs.length) e.sales_reference_orgs = 'Sales Reference Org is required.'
      if (!form.customer_search_term) e.customer_search_term = 'Search Term is required.'
      if (!form.customer_company_code) e.customer_company_code = 'Company Code is required.'
      if (!form.sales_organization.length) e.sales_organization = 'Sales Organization is required.'
      if (!form.distribution_channel) e.distribution_channel = 'Distribution Channel is required.'
      if (!form.division) e.division = 'Division is required.'
      if (!form.delivery_plant) e.delivery_plant = 'Delivery Plant is required.'
      if (!form.transportation_zone) e.transportation_zone = 'Transportation Zone is required.'
      if (!form.payment_terms) e.payment_terms = 'Payment Terms is required.'
    }
      ...
      <div className="modal-panel edit-panel">
        ...
        <div className="card" style={{ marginBottom: '1rem' }}>
          ...
          <div className="grid-2">
            ...
            <div className="field">
              <label>City</label>
              <input type="text" value={form.city} onChange={(e) => set('city', sanitizePlaceName(e.target.value))} placeholder="City" />
            ...
            <div className="field">
              ...
              <input type="text" value={form.district} onChange={(e) => set('district', sanitizeAddressText(e.target.value))} placeholder="District" />
            </div>
            <div className="field">
              <label>Country</label>
            ...
            <div className="field span-2">
              ...
              <input type="text" value={form.street1} onChange={(e) => set('street1', sanitizeAddressText(e.target.value))} maxLength={35} placeholder="Building / Plot No., Street Name" />
            </div>
            <div className="field">
      ...
      {panVerification?.sandbox_response?.data && (
        ...
        >
          ...
          <div>
            ...
            <div>
              {!isPanNameEditable(data.pan_number)
                ? "Not Applicable"
                : panVerification.sandbox_response.data
